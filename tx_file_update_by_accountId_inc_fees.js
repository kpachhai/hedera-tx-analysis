...
async function analyzeTransactions() {
  ...
  const fileName = "output/tx_file_update_by_accountId_inc_fees.csv";

  const txTypeStats = await collection
    .aggregate([
      {
        $match: {
          "body.fileUpdate": { $exists: true },
          sigCount: { $gt: 0 },
      ...
      },
      {
        $project: {
          ...
          fileSize: {
            $strLenCP: {
              $ifNull: ["$body.fileUpdate.contents", ""],
            },
          },
          numKeys: {
            $cond: {
              if: { $isArray: "$body.fileUpdate.keys" },
              then: { $size: "$body.fileUpdate.keys" },
              else: 0,
            },
          },
