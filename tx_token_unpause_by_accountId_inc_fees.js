...
async function analyzeTokenUnpauseTransactions() {
  const client = new MongoClient(uri);
  await client.connect();
  const db = client.db(process.env.DB);
  const collection = db.collection(process.env.COLLECTION);

  const fileName = "output/tx_token_unpause_by_accountId_inc_fees.csv";

  const txStats = await collection
    .aggregate([
      {
        $match: {
          "body.tokenUnpause.token.tokenNum": { $exists: true },
          "record.transactionFee": { $exists: true, $ne: null },
        },
      },
      {
        $project: {
      ...
      },
      {
        $project: {
          ...
          accountId: 1,
          feeInHbar: {
            $multiply: ["$transactionFee", "$exchRate", 0.0000000001],
          },
        },
      },
      {
...
analyzeTokenUnpauseTransactions();
