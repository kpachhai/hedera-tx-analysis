...
async function analyzeTransactions() {
  ...
  const fileName = "output/tx_ethereum_transaction_by_accountId_inc_fees.csv";

  const txTypeStats = await collection
    .aggregate([
      {
        $match: {
          "body.ethereumTransaction": { $exists: true },
          "record.transactionFee": { $exists: true },
          "record.contractCallResult.gasUsed": { $exists: true },
        },
      },
      {
        $project: {
          ...
          gasAmount: { $toInt: "$record.contractCallResult.gasUsed" },
          exchRate: {
            $divide: [
