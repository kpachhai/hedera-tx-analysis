...
async function analyzeTransactions() {
  ...
  const fileName = "output/tx_token_disassociate_by_accountId_inc_fees.csv";

  const txStats = await collection
    .aggregate([
      {
        $match: {
          "body.tokenDissociate": { $exists: true },
      ...
      },
      {
        $project: {
          ...
          disassociateCount: {
            $size: "$body.tokenDissociate.tokens",
          },
      ...
      },
      {
        $group: {
          ...
          txCount: { $sum: 1 },
          totalDisassociations: { $sum: "$disassociateCount" },
  ...
  await writeCSV(
    ...
    txStats.map((e) => ({
      ...
      txCount: e.txCount,
      disassociationCount: e.totalDisassociations,
      avgFee: e.avgFee,
      maxFee: e.maxFee,
      minFee: e.minFee,
    }))
