...
async function analyzeTransactions() {
  ...
  const fileName =
    "output/tx_crypto_approve_allowance_by_accountId_inc_fees.csv";

  const txTypeStats = await collection
    .aggregate([
      {
        $match: {
          "body.cryptoApproveAllowance": { $exists: true },
          sigCount: { $gt: 0 },
      ...
      },
      {
        $project: {
          ...
          ftApprovals: {
            $size: {
              $ifNull: ["$body.cryptoApproveAllowance.tokenAllowances", []],
            },
          },
          nftApprovals: {
            $size: {
              $ifNull: ["$body.cryptoApproveAllowance.nftAllowances", []],
            },
          },
          transactionFee: {
  ...
  await writeCSV(
    ...
    txTypeStats.map((e) => ({
      ...
      totalApprovals: e._id.totalApprovals,
      txCount: e.txCount,
      avgFee: e.avgFee,
      maxFee: e.maxFee,
      minFee: e.minFee,
