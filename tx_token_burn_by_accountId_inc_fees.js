...
async function analyzeTransactions() {
  ...
  const fileName = "output/tx_token_burn_by_accountId_inc_fees.csv";

  const txTypeStats = await collection
    .aggregate([
      {
        $match: {
          "body.tokenBurn": { $exists: true },
      ...
      },
      {
        $project: {
          ...
          burnAmount: {
            $toDouble: "$body.tokenBurn.amount",
          },
          transactionFee: {
      ...
      },
      {
        $group: {
          ...
          txCount: { $sum: 1 },
          totalBurnAmount: { $sum: "$burnAmount" },
  ...
  await writeCSV(
    ...
    txTypeStats.map((e) => ({
      ...
      txCount: e.txCount,
      totalBurnAmount: e.totalBurnAmount,
      avgFee: e.avgFee,
      maxFee: e.maxFee,
      minFee: e.minFee,
    }))
