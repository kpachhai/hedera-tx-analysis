...
async function analyzeTransactions() {
  ...
  const fileName = "output/tx_file_create_by_accountId_inc_fees.csv";

  const txTypeStats = await collection
    .aggregate([
      {
        $match: {
          "body.fileCreate": { $exists: true },
          sigCount: { $gt: 0 },
      ...
      },
      {
        $project: {
          ...
          fileSize: {
            $cond: {
              if: { $isArray: "$body.fileCreate.contents" },
              then: { $size: "$body.fileCreate.contents" },
              else: {
                $strLenCP: {
                  $ifNull: ["$body.fileCreate.contents", ""],
                },
              },
            },
          },
          numKeys: {
            $cond: {
              if: { $isArray: "$body.fileCreate.keys" },
              then: { $size: "$body.fileCreate.keys" },
              else: 0,
            },
          },
